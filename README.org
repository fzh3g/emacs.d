...
First time you boot Emacs, it will automatically install all the packages
defined in =lisp/init-elpa.el=. However, a warning about
=org-plus-contrib= may occur like:

#+BEGIN_EXAMPLE
  Error (use-package): org :config: Invalid function: org-babel-header-args-safe-fn
#+END_EXAMPLE

you have, to

- =M-x package-list-packages= and =C-h o= or =C-s= to search for
  =org-plus-contrib= and press =D= then =x= to delete the package.
- Relaunch Emacs to automatically install =org-plus-contrib= back, then
  exit Emacs.
- Launch Emacs again, no such error anymore.
...
one of my favorite extensions of Emacs. I use it for =M-x= and buffer-list,
...
| =C-x b=       | helm-mini (for buffer list and recent file)        |
| =C-h o=       | helm-occur (searching inside current buffer)       |
| =C-h i=       | helm-imenu (index of functions, variables, etc)    |
| =C-h <SPC>=   | helm-all-mark-rings                                |
| =C-c h x=     | helm-register                                      |
| =C-c h M-:=   | helm-eval-expression-with-eldoc                    |
| =C-x C-o=     | helm-find-files                                    |
| =C-c h g=     | helm-do-grep                                       |
| =C-h a=       | helm-apropos (describe functions, variables, etc ) |
| =C-h b=       | helm-bookmarks                                     |
| =C-h r=       | helm-mark-ring                                     |
| =C-h u=       | helm-resume                                        |
| =C-c h C-c w= | helm-wikipedia-suggest                             |
| =C-h k=       | helm-descbinds (describe keybindings)              |
| =M-s a a=     | helm-do-ag (code searching)                        |
| =M-s a f=     | helm-do-ag-this-file                               |
| =M-s a b=     | helm-do-ag-buffers                                 |
| =M-s a p=     | helm-do-ag-project-root                            |
| =M-s s=       | helm-swoop (code searching)                        |
| =M-s S=       | helm-multi-swoop                                   |
| =M-s C-s=     | helm-multi-swoop-all                               |
| =C-c y=       | helm-yas-complete (yasnippet integration)          |
